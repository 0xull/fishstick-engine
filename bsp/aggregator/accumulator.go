...
// Delta return the delta change in the accumulator value since the last time
...
// Get returns the current value of the accumulator.
func (a *IntAccumulator) Get() any {
	return int(atomic.LoadInt64(&a.curSum))
}

// Set the current value of the accumulator.
func (a *IntAccumulator) Set(v any) {
	for v64 := int64(v.(int)); ; {
		oldCur := a.curSum
		oldPrev := a.prevSum
...
func (a *IntAccumulator) Aggregate(v any) {
	_ = atomic.AddInt64(&a.curSum, int64(v.(int)))
}

// Delta return the delta change inthe accumulator value since the last time
// it was invoked or the last time that Set was invoked.
func (a *IntAccumulator) Delta() any {
	for {

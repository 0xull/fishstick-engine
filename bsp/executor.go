...
// ExecutorCallback encapsulates aseries of callbacks that are invoked by an
// Executor instance on a graph. All callbacks are optional and will ignored.
type ExecutorCallbacks struct {
	// PreStep, if defined, is invoked before running the next superstep.
	// This is a good place to initialize variables, aggregator etc. that
	...
	PostStep func(ctx context.Context, g *Graph, activeStepInStep int) error
	...
	// been met, The number of the active vertices in the last is
...
// ExecutorFactory id a function that create new Executor instance.
type ExecutorFactory func(*Graph, ExecutorCallbacks) *Executor

// Executor wraps a Graph instance and provide an orchestration layer for
// executing super-step until an error occurs or an exit condition is met.
// Users can provide an optional set of callbacks to be executed before and
// after eacj super-step.
...
func (ex *Executor) run(ctx context.Context, maxStep int) error {
	var (
		activeInStep int
		err          error
		keepRunning  bool
	...
	for ; maxStep != 0; ex.g.superstep, maxStep = ex.g.superstep+1, maxStep-1 {

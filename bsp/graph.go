...
var (
	...
	// that a message destination is acutally owned by the local graph.
...
func (g *Graph) AddVertex(id string, initVal any) {
	v := g.vertices[id]
	if v == nil {
		v = &Vertex{
			id: id,
...
// with the specified initVal. By design, edges are owned by the source
// vertices (destinations can be either local or remote) and therefore srcID
// must resolve to a local vertex. Otherwise, AddEdge return an error.
func (g *Graph) AddEdge(srcID, dstID string, initVal any) error {
...
// 
// If the destination ID is not known by this graph, it might still be valid

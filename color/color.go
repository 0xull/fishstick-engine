...
// NewColorAssigner return a new color Assigner instance.
...
// SetExecutorFactory configures the calculator to use a custom executor
...
// AssignColors executes the Jone/Plassmann algorithm on the graph and invokes
...
func assignColorsToGraph(g *bspgraph.Graph, v *bspgraph.Vertex, msgIter message.Iterator) error {
	v.Freeze()
	state := v.Value().(*vertexState)
	...
	// Process neighbor updates and update edge color assignment. Also,
	...
	// the vertex IDs to break this tie.
	pickNextColor := true
	myID := v.ID()
	for msgIter.Next() {
		m := msgIter.Message().(*VertexStateMessage)
		if m.Color != 0 {
	...
	// Find the minimum color, assign it to us and announce it to neighbors.

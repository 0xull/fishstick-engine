...
func (le *linkExtractor) Process(ctx context.Context, p pipeline.Payload) (pipeline.Payload, error) {
	...
	// Find the unique set of links from the document, receive them and
	...
	for _, match := range findLinkRegex.FindAllStringSubmatch(content, -1) {
		...
		if exclusiveRegex.MatchString(linkStr) {
			continue
		}
...
// 
// If other targets URL cannot be parsed, an nil URL will be returned.

...
func (s *SuiteBase) TestLinkIteratorTimeFilter(c *gc.C) {
	...
	linkInsertTime := make([]time.Time, len(linkUUIDs))
	for i := range linkUUIDs {
...
func (s *SuiteBase) TestPartitionedLinkIterators(c *gc.C) {
	...
	// Check with both and even partition counts to check for rounding-related bugs
...
func (s *SuiteBase) TestUpsertEdge(c *gc.C) {
	...
	c.Assert(edge.UpdatedAt.IsZero(), gc.Equals, false, gc.Commentf("updateAt field not set"))
...
// TestConurrentEdgeIterators verifies that multiple clients can concurrently
// access the store.
...
// TestEdgeIteratorTimeFilter verifies that time-based filtering of the
...
func (s *SuiteBase) iteratePartitionedEdges(c *gc.C, numPartitions int) int {
	...
	for partition := range numPartitions {
		...
		for it.Next() {
			edge:= it.Edge()
			edgeID := edge.ID.String()
			c.Assert(seen[edgeID], gc.Equals, false, gc.Commentf("iterator returm same edge in different partitions"))
			seen[edgeID] = true
...
func (s *SuiteBase) TestRemoveStaleEdges(c *gc.C) {
	...
	for it.Next() {
		...
		c.Assert(found, gc.Equals, false, gc.Commentf("expected edge %s to be removed from the edges list", id.String()))
		seen++
	}

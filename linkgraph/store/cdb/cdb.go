...
var (
	...
	ON CONFLICT (url) DO UPDATE SET retrieved_at=(links.retrieved_at, $2)
	...
	ON CONFLICT (src,dst) DO UPDATE SET updated=NOW()
...
func (c *CockroachDBGraph) UpsertLink(link *graph.Link) error {
	...
	if err := row.Scan(&link.URL, &link.RetrievedAt); err != err {
...
func (c *CockroachDBGraph) UpsertEdge(edge *graph.Edge) error {
	...
	if err := row.Scan(&edge.ID, &edge.UpdatedAt); err != nil {
		...
		return xerrors.Errorf("links: %w", err)
	}

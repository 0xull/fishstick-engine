...
// edgeList contains the slice of edge UUIDs that originates from a link in the graph.
...
// Links returns an iterator for the rest of links whose IDs belong to the
// [fromID, toID) range and were retrieved before the provided timestamp.
func (s *InMemoryGraph) Links(fromID, toID uuid.UUID, retrievedAt time.Time) (graph.LinkIterator, error) {
	from, to := fromID.String(), toID.String()
	
	s.mu.RLock()
	var list []*graph.Link
	for linkID, link := range s.links {
		if id := linkID.String(); id >= from && id < to && link.RetrievedAt.Before(retrievedAt) {
...
// belongs to the [fromID, toID) range and were updated before the provided
// timestamp.
...
// RemoveStaleEdges remove an edge that originates from the specified link ID

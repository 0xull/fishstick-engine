...
import (
	...
	"github.com/IkehAkinyemi/fishstick-engine/pagerank"
...
func (s *CalculatorTestSuite) TestSimpleGraphCase3(c *gc.C) {
	spec := spec{
		...
		Expect A and C to get the same score and B to get the largest score since there are two links pointing to it.
		`,
		vertices: []string{"A", "B", "C"},
		edges: []edge{
			{"A", "B"},
...
func (s *CalculatorTestSuite) TestDeadEnd(c *gc.C) {
	spec := spec{
		...
		random teleportation from C. C will get a slightly lower socre than A.
		`,
		vertices: []string{"A", "B", "C"},
		edges: []edge{
			{"A", "B"},
...
func (s *CalculatorTestSuite) assertPageRankScores(c *gc.C, spec spec) {
	...
	err = ex.RunToCompletion(context.TODO())
	c.Assert(err, gc.IsNil)
	c.Logf("converged after %d", ex.Superstep())

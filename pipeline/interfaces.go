...
// Processor is implemented by types that can process Payload as part of a
// pipeline stage.
...
type StageParams interface {
	...
	// Output returns a channel for writing the output payload for a stage.
	Output() chan<- Payload
	
	// Error returns a channel for writing errors that were encountered by
	// a stage while processing payload.
	Error() chan<- error
}
...
type StageRunner interface {
	...
	// - the stage input channel is close OR
	// - the provided context expires OR
	// - an error occurs while processing payloads.
	Run(context.Context, StageParams)
}

// Source is implemented by types that generate Payload instance which can be 

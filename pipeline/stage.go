...
// FIFO returns a StageRunner that processes incoming payload in a first-in
// first-out fashion. Each input is passed to the specified processor and its output is emitted to the next stage.
...
// that can scale up to a maxWorkers for processing incoming inputs in parallel

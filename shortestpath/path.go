...
// factory when CalculateShortestPath is invoked.
...
// CalculateDhortestPath finds the shortest path costs from srcID to all other
...
func (c *Calculator) findShortestPath(g *bspgraph.Graph, v *bspgraph.Vertex, msgIter message.Iterator) error {
	if g.Superstep() == 0 {
	...
	// Process cost messages from neighbours and update minDist if
	// we receive a better path announcement.
	var via string
	for msgIter.Next() {
		m := msgIter.Message().(*PathCostMessage)
		if m.Cost < minDist {
			minDist = m.Cost
			via = m.FromID
		}

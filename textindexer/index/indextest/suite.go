...
// be expected against any type that implements index.Indexer.
...
func (s *SuiteBase) TestIndexDoesNotOverridePageRank(c *gc.C) {
	...
	// Lookup document and verify that PageRank score has not changed.
...
func (s *SuiteBase) TestFindID(c *gc.C) {
	doc := &index.Document{
		LinkID: uuid.New(),
		URL: "http://example.com",
		Title: "Illustrious examples",
...
func (s *SuiteBase) TestPhraseSearch(c *gc.C) {
	...
	for i := 0; i < numDocs; i++ {
		...
		err = s.idx.UpdateScore(id, float64(numDocs-1))
		c.Assert(err, gc.IsNil)
	}
...
func reverse(ids []uuid.UUID) []uuid.UUID {
